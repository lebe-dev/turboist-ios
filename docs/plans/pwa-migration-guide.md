...
- [ ] **Создание подзадачи** — через parent_id
- [ ] **Древовидное отображение** — задачи с indent'ом по уровню вложенности
- [ ] **Свёртывание/развёртывание** подзадач (collapsed_ids, персистится на сервер)
- [ ] **Прогресс подзадач** — `completed_sub_task_count / sub_task_count`
- [ ] **Загрузка завершённых подзадач** — по запросу (`GET /api/tasks/:id/completed-subtasks`)
- [ ] **Навигация к родительской задаче** — из детального просмотра
