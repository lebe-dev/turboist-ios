...
- [x] **4 уровня**: P1 (срочный, красный), P2 (высокий, оранж), P3 (средний, синий), P4 (низкий, серый)
- [x] **Визуальная индикация** цветом
- [x] **Быстрая смена** приоритета из контекстного меню
- [x] **Фильтрация по приоритету** (в view "All Tasks")

### 7.4. Даты и повторения

- [x] **Установка даты** — date picker
- [x] **Recurring tasks** — пресеты (every day, every weekday, every week, every month) + произвольный ввод через `due_string`
- [x] **Визуальная индикация** даты:
  - Просроченные (overdue) — выделение красным
  - Сегодня — выделение оранжевым
  - Завтра — синим
  - Будущие — серым
  - Recurring — иконка повтора
- [x] **Трекинг откладываний** — показ `postpone_count` (сколько раз дата менялась)
- [x] **Быстрая установка даты** — "Сегодня", "Завтра", конкретные дни недели (в Weekly view)

### 7.5. Лейблы

- [x] **Просмотр лейблов задачи** — бейджи в строке задачи
- [x] **Добавление/удаление лейблов** — в детальном просмотре
- [x] **Batch update labels** — для нескольких задач сразу
- [x] **Label configs** — настройка `inherit_to_subtasks` (из AppConfig)
- [x] **Список всех лейблов** с подсчётом задач

### 7.6. Авто-лейблы

- [x] **Автоматическое добавление лейблов при создании** — по маске в заголовке:
  - `mask: "купить"` + `label: "покупки"` → задача с "Купить молоко" получает лейбл "покупки"
  - `ignoreCase: true` (default) — регистронезависимый поиск
- [x] **Наследование лейблов контекста** — если `context.inheritLabels = true`, лейблы контекста добавляются к новой задаче

### 7.7. Контексты

- [x] **Переключение контекста** — из бокового меню или переключателя
- [x] **Фильтрация задач** по активному контексту:
...
- [x] **Рекурсивное включение подзадач** — если родитель попадает в контекст, все подзадачи тоже
- [x] **Персистенция** активного контекста (`activeContextId` через `PATCH /api/state`)

### 7.8. Views (представления)

- [x] **All Tasks** — все задачи с фильтрами (priority, labels, links-only)
- [x] **Inbox** — задачи в Inbox-проекте, с предупреждением при overflow
- [x] **Today** — задачи на сегодня (+ overdue), группировка по частям дня
- [x] **Tomorrow** — задачи на завтра, группировка по частям дня
- [x] **Weekly** — задачи с weekly-лейблом, прогресс-бар лимита
- [x] **Backlog** — задачи с backlog-лейблом, прогресс-бар лимита
- [x] **Completed** — завершённые задачи за N дней

### 7.9. Части дня (Day Parts)

- [x] **Группировка задач Today/Tomorrow** по частям дня (утро, день, вечер, без времени)
  - Конфигурируемые диапазоны часов из `config.dayParts`
  - Задачи без лейбла дня → секция "Без времени"
- [x] **Заметки к частям дня** — текстовое поле у каждой секции (`dayPartNotes`)
...
- [x] **По приоритету** — мульти-выбор P1-P4
- [x] **По лейблу** — мульти-выбор из доступных лейблов
- [x] **Только со ссылками** (links_only) — regex `https?://\S+` в content
- [x] **Состояние фильтров** персистится через `PATCH /api/state { allFilters }`

### 7.14. Quick Capture

- [x] Быстрый ввод задачи с предустановленным родителем (из `config.quickCapture`)
- [x] Минимальный UI: только content + опциональная дата
- [x] Авто-фокус на клавиатуру

### 7.15. "Следующее действие" (Next Action)

- [x] **Триггер**: после завершения задачи, если:
  - У задачи есть подзадачи → предложить создать следующую подзадачу
  - У задачи есть родитель → предложить создать follow-up
- [x] **Предзаполнение**: лейблы наследуются от завершённой задачи
...
- [x] Показ `expires_at` на задачах (время до автоудаления)
- [x] Информирование если auto-remove `paused`
- [x] Автоудаление выполняется на бэкенде — iOS только отображает информацию
